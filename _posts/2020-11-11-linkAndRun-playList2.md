...
| #ai# | Không A cũng không B ⇒ A하지도 않고 B하지도 않다 |
...
| #aq# | Muốn A thì + (주어) + phải B ~ ⇒ A하고 싶으면 (주어가) B를 해야 한다 |
...
| #ar# | Để A thì + (주어) + phải B ~ ⇒ A하려면 (주어가) B를 해야 한다 |
...
| #as# | Bấy giờ tôi không ~ nữa rồi ⇒ 나는 이제 (더 이상) ~ 하지 않아요 |
...
| #at# | Tôi không còn ~ (nữa) ⇒ 나는 이제 (더 이상) ~ 하지 않아요 |
...
| #aw# | có người nào ~ không? ⇒ ~한 사람이 있어요? |
...
| #ax# | 주어 + vẫn không ~ à?  **\|** 주어 + vẫn chưa ~ à? ⇒ 주어는 여전히 ~하지 않아요 |
