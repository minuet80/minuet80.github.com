...
var SimpleCalendar = function () {
  ...
  _createClass(SimpleCalendar, [{
    ...
    value: function create() {
      ...
      header.innerHTML = header.innerHTML + '<div class="sc-actions"><span class="sc-return-today">현재일자</span></div>';
    ...
    value: function updateSize() {
      ...
      if (parseInt(width) < 500) {
        ...
        var _actions = this.arrayfrom(this.container.querySelectorAll('.sc-actions'));
        _actions.forEach(function (v, i) {
      ...
      if (parseInt(height) < 400) {
        ...
        var _items = this.arrayfrom(this.container.querySelectorAll('.sc-item'));
        var _weeks = this.arrayfrom(this.container.querySelectorAll('.sc-week-item'));
        _items.forEach(function (v, i) {
          v.querySelector('.day').classList.remove('sc-item-small');
        });
        _weeks.forEach(function (v, i) {
    ...
    value: function addData(year, month) {
      ...
      for (var _i5 = 0; _i5 < 35; _i5++) {
        ...
        var theday = new Date(Date.parse(thispageStart) + (_i5 - 1) * 24 * 3600 * 1000);
        var writeyear = theday.getFullYear();
        var writeday = theday.getDate();
        var writemonth = theday.getMonth() + 1;
        if (writemonth != month) {
          daysElement[_i5].classList.add('sc-othermenth');
        }
        daysElement[_i5].querySelector('.day').innerHTML = writeday;
        //判断是否添加阴历
        if (this._options.showLunarCalendar) {
          daysElement[_i5].querySelector('.lunar-day').innerHTML = new LunarHelp(writeyear, writemonth, writeday).getLunarDayName();
        } else {
          daysElement[_i5].querySelector('.lunar-day').innerHTML = '';
    ...
    value: function updateMark(year, month) {
      ...
      if (options.showMark) {
        (function () {
          var daysElement = _this.arrayfrom(_this.container.querySelectorAll('.sc-item'));
    ...
    value: function updateHoliday(year, month) {
      ...
      if (options.showHoliday) {
        (function () {
          var daysElement = _this2.arrayfrom(_this2.container.querySelectorAll('.sc-item'));
          var currentmonth = month - 1;
          //取得节日数据
          var data = _this2.languageData.vocation['data_' + year];
          if (data) {
            daysElement.forEach(function (v, i) {
              var day = +v.querySelector('.day').innerHTML;
              if (day == 1) currentmonth++;
    ...
    value: function updateTheme(theme) {
      ...
      if (this._options.theme.changeAble) {
        (function () {
          ...
          var weeks = _this3.arrayfrom(_this3.container.querySelectorAll('.sc-week-item'));
          var days = _this3.arrayfrom(_this3.container.querySelectorAll('.sc-item'));
          weeks.forEach(function (v, i) {
    ...
    value: function updateEvent() {
      ...
      yearadd.onclick = function () {
        //点击切换年份
        var currentyear = selectYear.value; //获取年份
        var currentmonth = selectMonth.value; //获取月份
